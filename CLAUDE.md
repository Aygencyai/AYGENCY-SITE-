...
- The first questionnaire screen is also the approved `EdenLeadCaptured.v1`
  sender. It stores work email, inquiry consent, and bounded attribution before
  the diagnostic. It must reuse the final application's UUID while keeping a
  separate capture event UUID and signing secret.
...
| EDEN_LEAD_CAPTURE_INGEST_URL | Server-only approved Eden early-capture Edge Function URL |
| EDEN_LEAD_CAPTURE_SIGNING_SECRET | Server-only early-capture HMAC secret, at least 32 characters |
