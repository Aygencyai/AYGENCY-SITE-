...
same-origin `POST /api/eden/applications` route. That route validates again,
constructs the locked event, signs exact bytes, and calls the approved Supabase
Edge Function. The browser never receives the HMAC secret, Supabase URL/key, CRM
operator secret, or database credentials.
...
Inquiry/application-processing consent is required. Sales follow-up is recorded
with the application. Marketing is a separate optional checkbox, off by default,
and emitted only when granted. The honeypot receives a neutral response and no
downstream call.
...
The website API returns `202` only after one of the two strict CRM acceptance
receipts. It includes application identity but not the Edge request ID or any
upstream response body. A notification is attempted only after a first-time
acceptance; exact duplicates suppress it, and notification failure cannot undo
the committed CRM transaction.
...
The widget uses Cloudflare's explicit renderer at
`api.js?render=explicit`, action `eden_application_submit`, and disables the
automatic hidden response field. Expired, error, and timeout callbacks clear the
form token. The Edge Function performs mandatory server-side Siteverify,
including exact action and hostname, and uses `event_id` as Siteverify's
idempotency key.
